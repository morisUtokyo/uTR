...
void count_occurrences_long_unit(char *S, int n, int *SA, int *C, int **OCC, char *unit, int unitLen, int *covered, int MIN_number_repetitions ){
    ...
    for(multimap<int, Alignment*>::iterator P = sorted_by_X.begin();
        P != sorted_by_X.end(); )
    {
            #ifdef DEBUG_count_occurrences_long_unit
            cout << "P\t";  P->second->print();
            #endif
        ...
        #ifdef DEBUG_count_occurrences_long_unit
        cout << "Before delete elements in count_occurrences_long_unit\n";
        #endif
    ...
    for(set<Alignment*>::iterator iter = set_of_alignments.begin(); iter != set_of_alignments.end(); iter++){
        ...
        #ifdef DEBUG_count_occurrences_long_unit
        cout << "End of count_occurrences_long_unit\n";
        #endif
}
...
void put_all_substrings(char *aS){
    std::string s(aS);
    int n = s.size();
    for(int i=0; i<n; i++)
        for(int j=1; j < MAX_UNIT_LENGTH && i+j < n; j++)
            put_unit(&(s.substr(i,j))[0]);
}
